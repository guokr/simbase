...
public class SimEngineImpl implements SimEngine, SimBasisListener {
	...
	private final ExecutorService mngmExec = Executors
	...
	private final ThreadPoolExecutor readerPool = new ThreadPoolExecutor(53,
			83, 37, TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(100),
	...
	public SimEngineImpl(SimContext simContext) {
		...
		this.savePath = new StringBuilder(System.getProperty("user.dir"))
				.append(separator).append(context.getString("savepath"))
	...
	private void validatePath(String filePath) throws SimEngineException {
		if (!new File(filePath).exists()) {
			throw new SimEngineException(String.format(
					"Dmp file '%s' not exists", filePath));
		}
	}
	...
	private void validateNotExistence(String toCheck) throws SimEngineException {
		if (basisOf.containsKey(toCheck)) {
			throw new SimEngineException(String.format(
					"Data entry '%s' already exists", toCheck));
		}
	}

	private void validateKind(String op, String toCheck, Kind kindShouldBe)
			throws SimEngineException {
		if (!kindOf.containsKey(toCheck)
				|| !kindShouldBe.equals(kindOf.get(toCheck))) {
			throw new SimEngineException(String.format(
					"Operation '%s' against a non-%s type '%s'", op,
					kindShouldBe, toCheck));
		}
	}
	...
	private void validateId(int toCheck) throws SimEngineException {
		if (toCheck < 1) {
			throw new SimEngineException(String.format(
					"Inviad id '%d', should be positive integer", toCheck));
		}
	}
	...
	private void validateProbs(float[] toCheck) throws SimEngineException {
		for (float prob : toCheck) {
			if (prob < 0 || prob > 1) {
				throw new SimEngineException(String.format(
						"Invaid prob '%s', should be between 0 and 1", prob));
			}
		}
	}

	private void validatePairs(int maxIndex, int[] toCheck)
			throws SimEngineException {
		...
		for (int offset = 0; offset < len; offset += 2) {
			if (toCheck[offset] < 0 || toCheck[offset] > maxIndex) {
				throw new SimEngineException(String.format(
						"Sparse matrix index '%d' out of bound",
						toCheck[offset]));
			}
			if (toCheck[offset + 1] < 0) {
				throw new SimEngineException(String.format(
						"Sparse matrix value '%d' should be non-negative",
						toCheck[offset + 1]));
			}
		}
	}
		...
		writerExecs.get(basisOf.get(key)).execute(new AsyncSafeRunner("del") {
			...
			public void delete(String toDel) {
				...
				case RECOMM:
					...
					basisOf.remove(toDel);
					kindOf.remove(toDel);
					logger.info(String.format("recommendation[%s] deleted",
							toDel));
					break;
				}
			}
	...
	public void bload(final SimCallback callback, final String bkey) {
		mngmExec.execute(new AsyncSafeRunner("bload") {
			...
			public void invoke() {
				...
				String filePath = new StringBuilder(savePath).append(bkey)
						.append(".dmp").toString();
	...
	public void blist(final SimCallback callback) {
		mngmExec.execute(new SafeRunner("blist", callback) {
			...
			public void invoke() {
				...
				callback.stringList((String[]) bkeys.toArray(new String[bkeys
						.size()]));
			}
		});
	}
	...
	@Override
	public void bmk(final SimCallback callback, final String bkey,
			final String[] base) {
	...
	@Override
	public void brev(final SimCallback callback, final String bkey,
			final String[] base) {
	...
	@Override
	public void vmk(final SimCallback callback, final String bkey,
			final String vkey) {
		mngmExec.execute(new SafeRunner("vmk", callback) {
			...
			public void invoke() {
				...
				logger.info(String.format(
						"vectorset[%s] created under basis[%s]", vkey, bkey));
	...
	@Override
	public void vget(final SimCallback callback, final String vkey,
			final int vecid) {
		validateKind("vget", vkey, Kind.VECTORS);
		final String bkey = basisOf.get(vkey);
	...
	@Override
	public void vadd(final SimCallback callback, final String vkey,
			final int vecid, final float[] vector) {
		...
		writerExecs.get(bkey).execute(new AsyncSafeRunner("vadd") {
			...
			public void invoke() {
				...
				if (counter % bycount == 0) {
					logger.info(String.format("adding dense vectors %d to %s",
							counter, vkey));
				}
			}
		});
	...
	@Override
	public void vset(final SimCallback callback, final String vkey,
			final int vecid, final float[] vector) {
		...
		writerExecs.get(bkey).execute(new AsyncSafeRunner("vset") {
			...
			public void invoke() {
				...
				counters.put(vkey, counter);
				if (counter % bycount == 0) {
					logger.info(String.format("setting dense vectors %d to %s",
							counter, vkey));
				}
			}
		});
	...
	@Override
	public void vacc(final SimCallback callback, final String vkey,
			final int vecid, final float[] vector) {
		...
		writerExecs.get(bkey).execute(new AsyncSafeRunner("vacc") {
			...
			public void invoke() {
				...
				if (counter % bycount == 0) {
					logger.info(String.format(
							"acculmulating dense vectors %d to %s", counter,
							vkey));
				}
			}
		});
	...
	@Override
	public void vrem(final SimCallback callback, final String vkey,
			final int vecid) {
		...
		writerExecs.get(bkey).execute(new AsyncSafeRunner("vrem") {
			...
			public void invoke() {
				...
				logger.info(String.format(
						"vector[%s] removed from vectorset[%s]", vecid, vkey));
			}
		});
	...
	@Override
	public void iget(final SimCallback callback, final String vkey,
			final int vecid) {
	...
	@Override
	public void iadd(SimCallback callback, final String vkey, final int vecid,
			final int[] pairs) {
		...
		writerExecs.get(bkey).execute(new AsyncSafeRunner("iadd") {
			...
			public void invoke() {
				...
				counters.put(vkey, counter);
				if (counter % bycount == 0) {
					logger.info(String.format("adding sparse vectors %d to %s",
							counter, vkey));
				}
			}
		});
	...
	@Override
	public void iset(final SimCallback callback, final String vkey,
			final int vecid, final int[] pairs) {
		...
		writerExecs.get(bkey).execute(new AsyncSafeRunner("iset") {
			...
			public void invoke() {
				...
				if (counter % bycount == 0) {
					logger.info(String.format(
							"setting sparse vectors %d to %s", counter, vkey));
				}
			}
		});
	...
	@Override
	public void iacc(final SimCallback callback, final String vkey,
			final int vecid, final int[] pairs) {
		...
		writerExecs.get(bkey).execute(new AsyncSafeRunner("iacc") {
			...
			public void invoke() {
				...
				if (counter % bycount == 0) {
					logger.info(String.format(
							"accumulating sparse vectors %d to %s", counter,
							vkey));
				}
			}
		});
	...
	public void rlist(final SimCallback callback, final String vkey) {
		mngmExec.execute(new SafeRunner("rlist", callback) {
			...
			public void invoke() {
				...
				callback.stringList((String[]) targets
	...
	@Override
	public void rmk(final SimCallback callback, final String vkeySource,
			final String vkeyTarget, final String funcscore) {
	...
	@Override
	public void rget(final SimCallback callback, final String vkeySource,
			final int vecid, final String vkeyTarget) {
		...
		readerPool.submit(new SafeRunner("rget", callback) {
			@Override
			public void invoke() {
				callback.stringList(bases.get(bkey).rget(vkeySource, vecid,
						vkeyTarget));
			}
		});
	}
	...
	@Override
	public void rrec(final SimCallback callback, final String vkeySource,
			final int vecid, final String vkeyTarget) {
		validateKind("rget", vkeySource, Kind.VECTORS);
		validateKind("rget", vkeyTarget, Kind.VECTORS);
		String rkey = rkey(vkeySource, vkeyTarget);
		...
		readerPool.submit(new SafeRunner("rrec", callback) {
			@Override
			public void invoke() {
				callback.integerList(bases.get(bkey).rrec(vkeySource, vecid,
						vkeyTarget));
			}
		});
	}
	...
	@Override
	public void listen(final String srcVkey, final String tgtVkey,
			final RecommendationListener listener) {
