	...
	public static int readSize(String line) {
		...
		while (idx < len - 1) {
			...
			if (digit >= 0 && digit < 10) {
				...
				String msg = String
						.format("Invalid character in the section for size: '%c'",
								next);
	...
	public static String readString(String line, int nbytes) {
		...
		if (len > nbytes) {
			String msg = String.format(
					"Invalid string legnth[%d] in the line: '%s'", len, line);
			throw new IllegalStateException(msg);
		}
		return builder.toString();
	}

	public RedisRequests decode(ByteBuffer buffer, RedisRequests last)
			throws ProtocolException, LineTooLargeException {
		byte discr;
		String line = null;
		state = State.INITIAL;
			...
			case READ_BEGIN:
				if (line != null && line.length() != 0) {
					...
					if (discr == SimUtils.STAR) {
						...
						String msg = String.format("wrong byte %s('%c')",
								discr, discr);
